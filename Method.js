	...
	return {
		...
		ajax: function (opts) {
			...
			let type = (opts.type || 'GET').toLowerCase(),
				...
				dataType = opt.dataType || 'json', //响应数据类型
				jsonp = opt.jsonp || 'callback',
				data = opt.data || {},
				params = fromatParams(data);
			...
			switch (type) {
				case 'get':
					XHR.open('get', opt.url + "?" + params, async);
					XHR.send(null);
					break;
				case 'post':
					XHR.open('post', opt.url, async);
				...
				case 'jsonp':
					...
					script.src = opt.url + '?' + params; // 发送请求，然后这个全局函数被调用window[fnName]
